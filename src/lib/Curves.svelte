...
<section class="schedule-card">
  ...
  <svg bind:this={svgEl} viewBox={`0 0 ${W} ${H}`} class="chart" role="application" aria-label="24-hour hardware brightness, overlay dimming, and temperature schedule" onpointermove={pointerMove} onpointerup={stopDrag} onpointercancel={stopDrag}>
    ...
    {/each}
    <text x="18" y={PLOT.top + PLOT.height / 2} class="axis-title left" transform={`rotate(-90 18 ${PLOT.top + PLOT.height / 2})`}>Brightness · 0–100%</text>
    <text x={W - 18} y={PLOT.top + PLOT.height / 2} class="axis-title right" transform={`rotate(90 ${W - 18} ${PLOT.top + PLOT.height / 2})`}>Temperature · warm → cool</text>
